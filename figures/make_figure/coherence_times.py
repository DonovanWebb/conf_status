...
for rid in rid_dict.keys():
    ...
    plt.vlines(
        ...
        color=c,
        linestyle=":",
        label=f"{rid_dict[rid]} $\\tau$ = {popt_ramsey[1]/1000:.1f}({p_err[1]/100:.0f}) $\mu$s",
        alpha=0.75,
    )
...
for rid in rid_dict.keys():
    ...
    num = (rid - 3) % 4
    c = graph_style.get_color(num)
    ax1.errorbar(delays, contrast, yerr=contrast_err, fmt="x", color=c)
    ...
    ax1.vlines(
        ...
        linestyle=":",
        label=r"$|3D_{5/2},~m_j =$ "
        + f"{rid_dict[rid]}$\\rangle$ $\\tau$ = {popt_ramsey[1]/1000:.1f}({p_err[1]/100:.0f}) ms",
        alpha=0.75,
    )
    # horizontal line at 1/e that ends at the fit line
...
coherence_times = [1568, 2257, 6179]
coherence_times_err = [166, 301, 298]
...
ax2.set_ylabel("$\\tau$ ($\mu$s)", fontsize=9)
ax1.text(
    -0.10,
    0.98,
    "b",
